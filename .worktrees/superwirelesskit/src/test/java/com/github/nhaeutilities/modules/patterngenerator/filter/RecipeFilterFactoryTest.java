...
public class RecipeFilterFactoryTest {
    ...
    public void buildAddsEnabledFiltersInPriorityOrder() {
        CompositeFilter filter = RecipeFilterFactory
            .build("(dustCopper)", "(ingotCopper)", "{NC Catalyst}", "{Bad Input}", "{Bad Output}", 4);
        ...
        assertEquals(
            ...
            filters.get(3)
                .getClass());
        assertEquals(
            OutputOreDictFilter.class,
            filters.get(4)
                .getClass());
        assertEquals(
            InputOreDictFilter.class,
            filters.get(5)
                .getClass());
    }
