...
NHAEUtilities 是一个专为 Minecraft 1.7.10 GregTech: New Horizons 整合包开发的实用辅助模组，提供三大功能模块：

- **样板生成器 (Pattern Generator)** — 基于可配置的过滤系统与平衡机制，将 GregTech 机器合成配方批量导出为标准的 AE2 编码样板
- **样板路由 (Pattern Routing)** — 自动将 AE2 编码样板路由至匹配的 GregTech 多方块输入仓，并自动配置仓内配方
- **超级无线设置工具 (Super Wireless Kit)** — 无需线缆或量子桥，即可将任意 AE2 频道设备无线连接至 ME 控制器
...
| 交互方式 | 功能说明 |
|----------|----------|
| **右键**（对空气） | 打开**主配置终端** — 填写机器 ID、过滤规则、指定电压等并提交生成请求 |
| **Shift + 右键**（对空气） | 打开**存储管理器** — 浏览并管理当前存储在虚拟节点的样板数据 |
| **Shift + 右键**（对方块） | **方块检测 / 条件导出** — 检测目标 GT 机器配方信息；若目标为可写入容器，则将虚拟样板导出到该容器 |
| **Shift + 右键**（安全终端） | 绑定 ME 网络，生成时可自动消耗网络内的空白样板 |

> 首次使用、重启后，或模组 / 配置变化后，请先在主界面点击 **Cache / 构建缓存**，待缓存完成后再进行浏览或生成。
...
| 交互方式 | 功能说明 |
|----------|----------|
| **右键**（对空气） | 打开**配方分析 GUI** — 分析配方映射，查看重复输入与最少样板组装数 |
| **Shift + 右键**（GT 机器） | 检测并保存该机器的配方映射，供后续分析与路由使用 |
...
| 模式 | 交互方式 | 功能说明 |
|------|----------|----------|
| QUEUE | **Shift + 右键**设备 | 将目标设备加入连接队列 |
| QUEUE | **潜行 + 左键**设备 | 批量递归收集相邻的频道设备 |
| BIND | **Shift + 右键**控制器 | 执行绑定，将队列中所有目标无线连接到该控制器 |
| 任意 | **右键**空气 | 在 QUEUE / BIND 模式之间切换 |
